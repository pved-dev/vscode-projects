...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_zero_sale_streak_v2")
HYPOTHESIS = (
    "Add zero_sale_streak feature: count of consecutive days with zero sales "
    "ending on the previous day. Uses fully vectorized groupby+cumsum approach "
    "without any Python loops. A new group counter resets on nonzero sales. "
    "Combined with current best settings (min_child_samples=100, lambda_l2=1.0, "
    "lambda_l1=0.1, num_leaves=256, lr=0.04, tweedie=1.5). Previous attempt "
    "exp_0080 failed — rewriting with simpler, safer vectorized implementation."
)
...
# Shift sales by 1 to get previous day's sales (avoid leakage)
df["sales_prev"] = df.groupby("id")["sales"].shift(1)
# nonzero flag: 1 if prev day had nonzero sales, else 0
df["nz_flag"] = (df["sales_prev"] > 0).astype(np.int32)
# cumsum of nonzero flags per item — resets group identifier
df["nz_cumsum"] = df.groupby("id")["nz_flag"].cumsum()
# streak = rank within each (id, nz_cumsum) group minus 1 gives days of zeros
# But simpler: streak = row_number - row_number_at_last_nonzero
df["row_num"] = df.groupby("id").cumcount()
# last nonzero position cummax trick
