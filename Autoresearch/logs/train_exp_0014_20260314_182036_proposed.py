"""
train.py — Enhanced LightGBM model for M5 Forecasting with expanding window features.

THIS FILE IS MODIFIED BY THE AGENT. Each run produces:
  results/preds_{RUN_ID}.parquet
  results/log_{RUN_ID}.json

Hypothesis: EXPANDING_WINDOWS — Add expanding window statistics (mean, std, min, max)
            to capture long-term trends and patterns that accumulate over time.
"""
import json
import os
import time
...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "expanding_windows")
HYPOTHESIS = (
    "EXPANDING_WINDOWS: Add expanding window statistics (mean, std, min, max) "
    "to capture long-term trends and patterns that accumulate over time."
)
