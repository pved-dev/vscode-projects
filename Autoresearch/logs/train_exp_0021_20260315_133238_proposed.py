...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_advanced_price_features")
HYPOTHESIS = (
    "LGB_ADVANCED_PRICE_FEATURES: Add sophisticated price-based features to better capture "
    "price elasticity and competitive dynamics. Include price percentile ranks within "
    "store/category/time periods, price volatility measures, price gap features relative to "
    "category averages, price elasticity proxies using price-sales correlations, and "
    "competitive pricing features. These should improve demand forecasting by better modeling "
    "how pricing decisions affect sales patterns across different product segments."
)
...
# ── 2c. Advanced price features
# Price percentile ranks within different groupings
df["price_rank_store"] = (
...
store_price_avg = df.groupby(["store_id", "d"])["sell_price"].transform("mean")
...
df["price_gap_store"] = (df["sell_price"] - store_price_avg).astype(np.float32)
...
# Price change momentum and acceleration
df["price_change_1"] = (
    df.groupby("id")["sell_price"]
...
df["price_trend_7"] = (
    df.groupby("id")["sell_price"]
      .transform(lambda x: x.rolling(7, min_periods=1).apply(lambda y: np.polyfit(range(len(y)), y, 1)[0] if len(y) > 1 else 0))
      .astype(np.float32)
)

# Competitive pricing features
df["price_vs_min_cat"] = (
    df["sell_price"] / (df.groupby(["cat_id", "d"])["sell_price"].transform("min") + 0.01)
).astype(np.float32)
df["price_vs_max_cat"] = (
    df["sell_price"] / (df.groupby(["cat_id", "d"])["sell_price"].transform("max") + 0.01)
).astype(np.float32)
...
# ── 2g. Target encoding for key categories
...
FEATURE_COLS = [
    ...
    "price_volatility_7", "price_volatility_28",
    "price_gap_cat", "price_gap_dept", "price_gap_store",
    "price_ratio_cat", "price_ratio_dept",
    "price_change_1", "price_change_7", "price_acceleration",
    "price_trend_7", "price_vs_min_cat", "price_vs_max_cat",
