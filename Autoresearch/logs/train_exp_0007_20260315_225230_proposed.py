...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_xgb_blend_v1")
HYPOTHESIS = (
    "Blend LightGBM and XGBoost predictions (equal weight 0.5/0.5) using the same "
    "full feature set as baseline (exp_0000). LightGBM uses tweedie power=1.5, "
    "num_leaves=256, lr=0.04. XGBoost uses reg:tweedie with tweedie_variance_power=1.5, "
    "max_depth=8, lr=0.04, n_estimators=800 with early stopping. "
    "Hypothesis: blending two diverse models reduces variance and improves WRMSSE."
)
