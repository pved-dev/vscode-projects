...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_price_momentum_v1")
HYPOTHESIS = (
    "Price momentum features: add price_change_direction (up/down vs last week as +1/-1/0), "
    "discount_depth = (price_max_52w - sell_price) / (price_max_52w + 0.01), "
    "and price_max_52w itself. These capture promotional patterns and price position "
    "relative to yearly high, which should help model discount-driven sales spikes. "
    "Keep best known params from exp_0090 (lambda_l2=10, lambda_l1=1.5, min_child_samples=300) "
    "with LGB+XGB blend at 0.55/0.45."
)
...
# ── 2i-NEW. Price momentum features (new hypothesis) ─────────────────────────
# price_change_direction: +1 if price went up vs 7 days ago, -1 if down, 0 if same
...
# 52-week (364-day) rolling max price — discount depth relative to yearly high
df["price_max_52w"] = (
    df.groupby("id")["sell_price"]
...
# discount_depth: how far current price is below 52-week high (0 = at max, 1 = free)
df["discount_depth"] = (
    (df["price_max_52w"] - df["sell_price"]) / (df["price_max_52w"] + 0.01)
).clip(lower=0).astype(np.float32)

# price_vs_52w_max ratio
df["price_vs_52w_max"] = (
    df["sell_price"] / (df["price_max_52w"] + 0.01)
).astype(np.float32)
...
log = {
    ...
    "new_features":           ["price_change_direction", "price_max_52w", "discount_depth", "price_vs_52w_max"],
}
