"""
train.py — Baseline LightGBM model for M5 Forecasting.

THIS FILE IS MODIFIED BY THE AGENT. Each run produces:
  results/preds_{RUN_ID}.parquet
  results/log_{RUN_ID}.json

Hypothesis: BASELINE — lag features [7, 28] + rolling mean [7, 28]
            + calendar and price features, LightGBM with default params.
"""
import json
import os
import time
...
# ── 2d. Snap feature: use the state-specific snap column
def get_snap(row):
    """Return the relevant snap flag for this row's state."""
    return None  # replaced below by vectorised approach

state_map = {"CA": "snap_CA", "TX": "snap_TX", "WI": "snap_WI"}
# state_id was label-encoded; map back — crude but works for baseline
# We'll just keep all three snap columns; the model will figure it out.
