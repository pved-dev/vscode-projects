...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_num_leaves_256_deeper_trees")
HYPOTHESIS = (
    "The best model so far uses num_leaves=256 (WRMSSE=0.94669). "
    "Hypothesis: increase max_depth to control tree structure more explicitly "
    "alongside num_leaves=256. Currently max_depth is unconstrained (-1). "
    "Setting max_depth=8 may improve generalization by preventing very deep paths "
    "that overfit with 256 leaves. Also try slightly lower learning_rate=0.04 "
    "with n_estimators=800 to allow more careful learning. "
    "All other settings match best run (exp_0029)."
)
...
# ── 2g. Day-of-week seasonal rolling means ────────────────────────────────────
