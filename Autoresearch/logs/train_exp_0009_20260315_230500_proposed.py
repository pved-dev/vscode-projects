...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_xgb_blend_regularize")
HYPOTHESIS = (
    "Try stronger regularization in LightGBM: increase lambda_l2 from 1.0 to 5.0, "
    "lambda_l1 from 0.1 to 0.5, min_child_samples from 100 to 200, and add "
    "min_gain_to_split=0.01. Keep 50/50 blend with XGBoost (same as exp_0003 best). "
    "Hypothesis: reducing overfitting via stronger regularization will improve WRMSSE."
)
...
# ── 4. Train LightGBM with stronger regularization ───────────────────────────
print("[train] Training LightGBM with stronger regularization …")
...
lgb_params = {
    ...
    "min_gain_to_split":      0.01,      # added non-zero threshold
    "lambda_l2":              5.0,       # increased from 1.0 to 5.0
    "lambda_l1":              0.5,       # increased from 0.1 to 0.5
    "verbosity":              -1,
    "n_jobs":                 -1,
    "seed":                   42,
}
...
# Encode categoricals for XGBoost (label encode)
...
for col in cat_cols:
    cats = X_train_xgb[col].astype("category")
    cat_map = {v: i for i, v in enumerate(cats.cat.categories)}
    X_train_xgb[col] = cats.cat.codes.astype(np.int32)
    X_valid_xgb[col] = X_valid_xgb[col].map(cat_map).fillna(-1).astype(np.int32)
    X_val_xgb[col]   = X_val_xgb[col].map(cat_map).fillna(-1).astype(np.int32)
...
xgb_params = {
    ...
    "subsample":               0.7,
    "min_child_weight":        200,      # matched to LGB min_child_samples
    "reg_lambda":              5.0,      # matched to LGB lambda_l2
    "reg_alpha":               0.5,      # matched to LGB lambda_l1
    "n_jobs":                  -1,
    "seed":                    42,
    "verbosity":               0,
}
...
dvalid_xgb = xgb.DMatrix(X_valid_xgb, label=y_valid)
...
xgb_preds = xgb_model.predict(dval_xgb, iteration_range=(0, xgb_model.best_iteration))
xgb_preds = np.clip(xgb_preds, 0, None).astype(np.float32)
print(f"[train] XGBoost best iteration: {xgb_model.best_iteration}")

# ── 6. Blend predictions (50/50 — same as best exp_0003) ─────────────────────
print("[train] Blending LightGBM and XGBoost predictions (50/50) …")
blend_preds = (0.5 * lgb_preds + 0.5 * xgb_preds).astype(np.float32)

val_df = val_df.copy()
val_df["pred"] = blend_preds
