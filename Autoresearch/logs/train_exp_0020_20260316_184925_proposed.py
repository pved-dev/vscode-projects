...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_seed_ensemble_v1")
HYPOTHESIS = (
    "Train 3 LightGBM models with seeds 42, 123, 777 using best known params "
    "(lambda_l1=0.5, lambda_l2=8.0, num_leaves=256, lr=0.04, tweedie_power=1.5) "
    "and average their predictions. Ensemble diversity from different random seeds "
    "should reduce variance and improve WRMSSE beyond single model 0.94333. "
    "Skip XGBoost to stay within time budget — LGB x3 seeds only."
)
...
# ── 4. Base LightGBM params (best known: lambda_l1=0.5) ──────────────────────
lgb_base_params = {
    "objective":              "tweedie",
...
dvalid = lgb.Dataset(X_valid_all, label=y_valid_all, reference=dtrain)

for seed in SEEDS:
    elapsed = time.time() - start_time
    print(f"[train] Training LGB seed={seed} (elapsed {elapsed:.1f}s) …")
    ...
    print(f"[train] Seed {seed} best iteration: {model.best_iteration}")

# ── 5. Average predictions across seeds ──────────────────────────────────────
print("[train] Averaging predictions across 3 seeds …")
final_preds = np.mean(all_preds, axis=0).astype(np.float32)
...
log = {
    ...
    "lgb_base_params":  lgb_base_params,
    "seeds":            SEEDS,
