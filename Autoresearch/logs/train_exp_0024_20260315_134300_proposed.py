...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_target_encoding_smoothing")
HYPOTHESIS = (
    "LGB_TARGET_ENCODING_SMOOTHING: Improve target encoding with Bayesian smoothing to prevent "
    "overfitting on rare categories. Use smoothed target encoding with global mean regularization "
    "based on category frequency. Add cross-validation-based target encoding to reduce leakage. "
    "This should improve generalization on categories with limited training examples while "
    "maintaining predictive power for well-represented categories."
)
...
# ── 2g. Advanced smoothed target encoding with cross-validation
TARGET_ENCODING_COLS = ["item_id", "store_id", "dept_id", "cat_id"]
train_mask = df["d"] < VAL_START_DAY
train_data = df[train_mask].copy()
global_mean = train_data["sales"].mean()

# Smoothing parameter - higher values = more smoothing (regularization)
SMOOTHING_ALPHA = 50.0

def bayesian_smoothing(group_data, global_mean, alpha):
    """Apply Bayesian smoothing to target encoding"""
    group_size = len(group_data)
    group_mean = group_data["sales"].mean()
    
    # Smoothed estimate: weighted average of group mean and global mean
    smoothed_mean = (group_size * group_mean + alpha * global_mean) / (group_size + alpha)
    return smoothed_mean

# Cross-validation based target encoding to reduce leakage
N_FOLDS = 5
...
for col in TARGET_ENCODING_COLS:
    ...
    # Calculate smoothed encoding for each fold
    ...
    # Fill any remaining NaN values (from validation period)
    # Use smoothed encoding from full training data
...
# ── 2h. Additional target encoding features - frequency-based smoothing
for col in TARGET_ENCODING_COLS:
    # Calculate category frequency in training data
    freq_map = train_data[col].value_counts().to_dict()
    df[f"{col}_freq"] = df[col].map(freq_map).fillna(1).astype(np.float32)
    
    # Create frequency-based confidence score
    df[f"{col}_confidence"] = np.minimum(df[f"{col}_freq"] / 100.0, 1.0).astype(np.float32)
...
FEATURE_COLS = [
    ...
    "item_id_freq", "store_id_freq", "dept_id_freq", "cat_id_freq",
    "item_id_confidence", "store_id_confidence", "dept_id_confidence", "cat_id_confidence"
]
