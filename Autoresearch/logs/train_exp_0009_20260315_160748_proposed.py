...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_price_features_v2")
HYPOTHESIS = (
    "Add richer price history features: rolling price mean/std over 7 and 28 days, "
    "price change from 7 days ago, and a 'price is at minimum in last 28 days' binary flag. "
    "These capture promotional pricing patterns that drive spikes in sales for high-revenue "
    "items — which matter most for WRMSSE. All other settings unchanged from exp_0005."
)
...
# ── 2f-NEW. Price history features — rolling price stats per item
# These capture promotional pricing patterns (price drops drive sales spikes)
...
# Price change from 7 days ago (captures promotional cycles)
df["price_change_7"] = (
    df.groupby("id")["sell_price"]
      .transform(lambda x: x - x.shift(7))
      .astype(np.float32)
)
# Price change from 28 days ago
df["price_change_28"] = (
    df.groupby("id")["sell_price"]
      .transform(lambda x: x - x.shift(28))
      .astype(np.float32)
)
# Binary flag: is current price at or below rolling 28-day minimum (promotion signal)
...
# Price relative to its own 28-day rolling mean (normalized price level)
df["price_norm_28"] = (
...
# ── 2k. Ratio of short-term to long-term rolling mean (momentum signal)
