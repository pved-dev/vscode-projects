...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "baseline_lgb_simple")
HYPOTHESIS = (
    "BASELINE_LGB_SIMPLE: Establish a clean baseline with LightGBM using "
    "essential features: basic lags [7, 14, 28], rolling means/stds [7, 28], "
    "price features, calendar features, and target encoding. No ensemble, "
    "no extreme clipping - just solid fundamentals to measure against."
)
...
df, sales_wide, weights, scale = build_dataset(max_lags=28)
