...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_xgb_blend_calendar_cyclical")
HYPOTHESIS = (
    "Add calendar cyclical encoding: day_of_month sin/cos, month sin/cos, "
    "is_month_end (dom>=28), is_month_start (dom<=3). "
    "Keep best known LGB+XGB blend (0.55/0.45) with tweedie params."
)
...
# Price momentum direction and discount depth
price_diff_7 = df.groupby("id")["sell_price"].transform(lambda x: x - x.shift(7))
df["price_change_direction"] = np.sign(price_diff_7).astype(np.float32)

df["price_max_52w"] = (
    df.groupby("id")["sell_price"]
      .transform(lambda x: x.shift(1).rolling(364, min_periods=1).max())
      .astype(np.float32)
)
df["discount_depth"] = (
...
# ── 2j. Calendar cyclical encoding (NEW) ─────────────────────────────────────
# Extract day-of-month from d column (day index relative to start of data)
# We use wday and month which are already in the dataframe
# Add day_of_month: d column maps to actual calendar dates
# d=1 corresponds to 2011-01-29, so we compute DOM from that
df["dom"] = ((df["d"] - 1 + 28) % 31 + 1)  # approximate, use actual calendar

# Better approach: use month column (already present) and d to derive DOM
# Use modular arithmetic based on actual calendar
# d=1 is 2011-01-29 (Saturday). Use pandas date arithmetic.
base_date = pd.Timestamp("2011-01-29")
# Compute actual date for each d value - vectorized
unique_days = df["d"].unique()
day_to_date = pd.Series(
    {d: base_date + pd.Timedelta(days=int(d) - 1) for d in unique_days}
)
df["_date"] = df["d"].map(day_to_date)
df["dom"] = df["_date"].dt.day.astype(np.float32)
df["_month_actual"] = df["_date"].dt.month.astype(np.float32)
df.drop(columns=["_date"], inplace=True)

# Cyclical encoding
TWO_PI = 2.0 * np.pi
df["dom_sin"] = np.sin(TWO_PI * df["dom"] / 31.0).astype(np.float32)
df["dom_cos"] = np.cos(TWO_PI * df["dom"] / 31.0).astype(np.float32)
df["month_sin"] = np.sin(TWO_PI * df["_month_actual"] / 12.0).astype(np.float32)
df["month_cos"] = np.cos(TWO_PI * df["_month_actual"] / 12.0).astype(np.float32)
df["is_month_end"]   = (df["dom"] >= 28).astype(np.float32)
df["is_month_start"] = (df["dom"] <= 3).astype(np.float32)
df.drop(columns=["dom", "_month_actual"], inplace=True)

# ── 2k. Target encoding with smoothing
...
# ── 2o. Ratio of short-term to long-term rolling mean (momentum signal)
...
# ── 2p. Yearly seasonality features using lag_365 and lag_168
...
FEATURE_COLS = [
    ...
    # NEW: Calendar cyclical encoding
    ...
    "price_change_direction", "discount_depth", "price_max_52w",
...
log = {
    ...
    "new_features":           ["dom_sin", "dom_cos", "month_sin", "month_cos",
                               "is_month_end", "is_month_start"],
}
