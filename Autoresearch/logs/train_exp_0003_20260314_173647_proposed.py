"""
train.py — Enhanced LightGBM model for M5 Forecasting with exponential smoothing features.

THIS FILE IS MODIFIED BY THE AGENT. Each run produces:
  results/preds_{RUN_ID}.parquet
  results/log_{RUN_ID}.json

Hypothesis: EWM_FEATURES — Add exponential weighted moving averages [7, 28] with alpha=0.3
            to capture recent trend momentum better than simple rolling means.
"""
import json
import os
import time
...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "ewm_features")
HYPOTHESIS = (
    "EWM_FEATURES: Add exponential weighted moving averages [7, 28] with alpha=0.3 "
    "to capture recent trend momentum better than simple rolling means."
)
