...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_dow_seasonal_lags")
HYPOTHESIS = (
    "Add day-of-week seasonal lag features: for each item, compute the rolling mean "
    "of sales on the same weekday over the past 4 and 8 occurrences (lag_7*4=28 days, "
    "lag_7*8=56 days worth of same-weekday observations). These capture strong weekly "
    "seasonality patterns more directly than generic rolling means, which average across "
    "all weekdays. Also add same-weekday lag features: lag_7, lag_14, lag_21, lag_28 "
    "already exist; add lag_35 and lag_49 (7-week) for same-weekday pattern. "
    "All other settings unchanged from exp_0011 (best so far)."
)
...
# ── 2a. Lag features
LAG_DAYS = [1, 2, 3, 7, 14, 21, 28, 35, 42, 49, 56]
for lag in LAG_DAYS:
...
# ── 2g. Day-of-week seasonal rolling means (NEW) ──────────────────────────────
# For each item, compute rolling mean of sales on the same weekday
# Uses lag_7, lag_14, lag_21, lag_28 (same-weekday observations already shifted)
# Rolling mean over 4 same-weekday observations = 4 weeks back
# Rolling mean over 8 same-weekday observations = 8 weeks back
print("[train] Computing day-of-week seasonal rolling means …")

# Stack same-weekday lags and compute their mean/std
same_wday_lags_4 = ["lag_7", "lag_14", "lag_21", "lag_28"]
same_wday_lags_8 = ["lag_7", "lag_14", "lag_21", "lag_28", "lag_35", "lag_42", "lag_49", "lag_56"]

# Mean of last 4 same-weekday observations
df["dow_roll_mean_4w"] = (
    df[same_wday_lags_4].mean(axis=1, skipna=True).astype(np.float32)
)
# Mean of last 8 same-weekday observations
df["dow_roll_mean_8w"] = (
    df[same_wday_lags_8].mean(axis=1, skipna=True).astype(np.float32)
)
# Std of last 4 same-weekday observations (captures weekday volatility)
df["dow_roll_std_4w"] = (
    df[same_wday_lags_4].std(axis=1, skipna=True).fillna(0).astype(np.float32)
)
# Trend: recent same-weekday vs older same-weekday
df["dow_trend_4w_8w"] = (df["dow_roll_mean_4w"] - df["dow_roll_mean_8w"]).astype(np.float32)
# Ratio of most recent to 4-week average same-weekday
df["dow_ratio_lag7_mean4w"] = (df["lag_7"] / (df["dow_roll_mean_4w"] + 0.01)).astype(np.float32)

# ── 2h. Price features (existing)
df["price_momentum"] = (
...
# ── 2i. Target encoding with smoothing
...
# ── 2m. Ratio of short-term to long-term rolling mean (momentum signal)
...
FEATURE_COLS = [
    ...
    # Day-of-week seasonal features (NEW)
