...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_tweedie_power")
HYPOTHESIS = (
    "Tune Tweedie variance power from 1.1 to 1.5 — retail sales follow a more "
    "heavily zero-inflated distribution, so a higher power may better fit the "
    "target distribution and reduce WRMSSE. All other settings unchanged from baseline."
)
...
FEATURE_COLS = [
    ...
    "lag_ratio_7_14", "lag_ratio_14_28", "lag_ratio_1_7",
...
# Key change: tweedie_variance_power = 1.5 (vs 1.1 in baseline)
# Higher power places more weight on zero/near-zero observations, better matching
# retail sales distribution which is heavily zero-inflated
lgb_params = {
    "objective":              "tweedie",
