"""
train.py — Enhanced LightGBM model for M5 Forecasting with more lag features.

THIS FILE IS MODIFIED BY THE AGENT. Each run produces:
  results/preds_{RUN_ID}.parquet
  results/log_{RUN_ID}.json

Hypothesis: EXPANDED_LAGS — Add more lag features [7, 14, 21, 28] instead of just [7, 28]
            to capture weekly and multi-weekly sales patterns.
"""
import json
import os
import time
...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "expanded_lags")
HYPOTHESIS = (
    "EXPANDED_LAGS: Add more lag features [7, 14, 21, 28] instead of just [7, 28] "
    "to capture weekly and multi-weekly sales patterns."
)
