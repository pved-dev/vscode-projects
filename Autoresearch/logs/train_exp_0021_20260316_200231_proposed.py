...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_xgb_blend_price_momentum")
HYPOTHESIS = (
    "Add price momentum features: price_change_direction (1 if price went up vs last week, "
    "-1 down, 0 same) and discount_depth = (price_max_52w - sell_price) / price_max_52w. "
    "Keep best known LGB+XGB blend (0.55/0.45) with tweedie params from exp_0093."
)
...
# ── 2i-NEW. Price momentum direction and discount depth ───────────────────────
# price_change_direction: 1 if price went up vs last week, -1 if down, 0 if same
...
# discount_depth: how deep a discount relative to 52-week max price
# Use rolling 364-day (52-week) max of sell_price, shifted by 1 to avoid leakage
df["price_max_52w"] = (
    df.groupby("id")["sell_price"]
...
# Clip to [0, 1] — discount can't be negative (current > historical max is ok edge case)
df["discount_depth"] = df["discount_depth"].clip(lower=0.0, upper=1.0).astype(np.float32)
...
FEATURE_COLS = [
    ...
    # NEW: price momentum direction and discount depth
...
log = {
    ...
    "new_features":           ["price_change_direction", "discount_depth", "price_max_52w"],
}
