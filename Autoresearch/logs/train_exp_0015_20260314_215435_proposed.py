...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "rolling_expand_features")
HYPOTHESIS = (
    "ROLLING_EXPAND: Add expanding window features (mean, std) to complement "
    "existing rolling windows. Expanding features capture long-term trends "
    "and seasonality that fixed-window rolling stats might miss."
)
...
# ── 2c. NEW: Expanding window features
df["expand_mean"] = (
    df.groupby("id")["sales"]
