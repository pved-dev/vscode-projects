...
# ── Run metadata ─────────────────────────────────────────────────────────────
RUN_ID    = os.environ.get("RUN_ID", "lgb_xgb_blend_v2")
HYPOTHESIS = (
    "Refine LGB+XGB blend from exp_0064 (WRMSSE=0.94306). "
    "Try different blend weights: 0.55 LGB + 0.45 XGB instead of 0.6/0.4. "
    "Keep LightGBM params: num_leaves=256, lr=0.04, lambda_l2=8.0, min_child_samples=250, tweedie_power=1.5. "
    "Keep XGBoost params same. Test if slightly more XGB weight improves blend diversity."
)
...
# Fill NaNs for XGBoost (it handles them natively but let's be safe)
...
# ── 5. Blend predictions ──────────────────────────────────────────────────────
# Test 0.55 LGB + 0.45 XGB (slightly more XGB weight than best 0.6/0.4)
WEIGHT_A = 0.55
WEIGHT_B = 0.45
